...
const AdminPanelContext =
  createContext<AdminPanelContextType | null>(
    null
  );

export const useAdminPanel =
  (): AdminPanelContextType => {
    const ctx =
      useContext(AdminPanelContext);

    if (!ctx) {
      throw new Error(
        "useAdminPanel must be used inside AdminPanelTemplate"
      );
    }
...
export default function AdminPanelTemplate({
  ...
  `}
        ...
        <main className="md:ml-64 min-h-screen text-on-surface">
          ...
          <div className="py-8 max-w-7xl mx-12 space-y-10">
            {children}
          </div>
        </main>
      </div>
