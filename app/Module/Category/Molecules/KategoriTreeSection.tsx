...
export function KategoriTreeSection() {
  const { state, updateTree } = useCategoryContext();
  ...
  return (
    <div className="bg-surface-container-lowest rounded-xl p-4 sm:p-6 lg:p-8 group/tree-item">
      <TreeView
        data={treeData}
        onChange={(data: TreeItem[]) => transformData(data)}
      />
      <div className="mt-12 pt-8 border-t border-surface-container flex justify-between items-center">
        ...
        <button
          ...
          className="bg-gradient-to-r from-primary to-primary-dim text-on-primary font-headline font-extrabold px-10 py-4 rounded-xl shadow-lg hover:shadow-primary/20 hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center gap-3 disabled:opacity-40"
        >
          Update Changes
        </button>
      </div>
